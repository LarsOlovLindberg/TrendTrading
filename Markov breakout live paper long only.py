# -*- coding: utf-8 -*-
"""
Markov Breakout – LIVE PRICES + PAPER TRADING (ingen riktig order skickas)
---------------------------------------------------------------------------
Detta skript läser live-priser från Binance (public API – kräver inga nycklar)
och kör din breakout-/BE-logik i "paper mode". All orderutförande simuleras
lokalt och balans hålls i minnet. L sätts till senaste exit-pris, enligt din
strategi.

Byt senare till riktiga order genom att sätta ORDER_TEST=false i config.json
och ersätta PaperBroker-anropen med riktiga Binance-anrop (markerat nedan).
...
# === Globalt tillstånd ========================================================
pos = Position()
paper = PaperBroker({"USDT": str(cfg.get("paper_usdt", "10000")), "BTC": "0"})

# Startnivå L = senast kända pris (kan även tas från config om du vill)
...
def maybe_enter(price: Decimal):
    """Brytlogik: upp genom L => LONG. (SHORT disabled för spot trading)"""
    global L
    if pos.side == "FLAT":
        if price > L:
            # ENTER LONG
            ...
            # SHORT disabled för spot trading (kräver futures/margin)
            print(f"⏭️  SKIP SHORT @ {price:.2f} (LONG-only mode för spot trading)")
            pass
...
def main():
    ...
    while True:
        try:
            ...
            if tick_count % 20 == 0:
                print(f"ℹ️  Pris: {price:.2f} | L: {L:.2f} | Pos: {pos.side} | Balans: USDT={paper.balances['USDT']:.2f} BTC={paper.balances['BTC']:.8f}")
