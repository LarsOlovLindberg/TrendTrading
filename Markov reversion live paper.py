# -*- coding: utf-8 -*-
"""
Markov REVERSION – LIVE PRICES + PAPER TRADING (MEAN REVERSION)
----------------------------------------------------------------
INVERTERAD STRATEGI: Satsar på att priset ska ÅTERVÄNDA till L-linjen!

- Läser RIKTIGA marknadspriser från Binance (public API – kräver inga nycklar)
- Kör MEAN REVERSION-logik i PAPER MODE (inga riktiga orders)
- Startband: två L-startlinjer (övre/nedre). Första brott:
    • öppna position MOTSATT riktning (SHORT vid brott uppåt, LONG vid brott nedåt)
    • sätt L = entry-priset
    • dölj startlinjerna
- TP = Återgång till L-linjen (det som tidigare var stop loss)
- STOP LOSS = Priset fortsätter bort från L (det som tidigare var TP)
- Progressiv scaling: Öka position när priset går fel håll (mot stop)
- Grafik: pris + aktiva linjer (L/TP/SL) + kompakt infobox
- CSV-logg: orders_paper.csv + session_summary.csv

Kör:
    python "Markov reversion live paper.py"
"""
...
def maybe_exit(price: Decimal):
    """EXIT vid L-korsning: Priset korsar L = exit och öppna ny position åt andra hållet"""
    global L
    qty = pos.qty if pos.qty > 0 else ORDER_QTY
    
    if pos.side == "LONG" and pos.entry is not None:
        # LONG: Vi är under L, exiterar när priset korsar UPP genom L
        if price >= L:
            print(f"✅ LONG EXIT: Priset {price:.2f} korsade L {L:.2f} uppåt → SHORT")
            if ORDER_TEST:
                paper.market_sell(SYMBOL, qty, price)
            do_exit("LONG", price, "LW")
            # Flytta L till korsningspunkten
            L = price
            # Öppna SHORT direkt (priset är nu över L)
            enter_short(price)
            refresh_lines(price)

    elif pos.side == "SHORT" and pos.entry is not None:
        # SHORT: Vi är över L, exiterar när priset korsar NER genom L
        if price <= L:
            print(f"✅ SHORT EXIT: Priset {price:.2f} korsade L {L:.2f} nedåt → LONG")
            if ORDER_TEST:
                paper.market_buy(SYMBOL, qty, price)
            do_exit("SHORT", price, "SW")
            # Flytta L till korsningspunkten
            L = price
            # Öppna LONG direkt (priset är nu under L)
            enter_long(price)
            refresh_lines(price)
...
def maybe_enter(price: Decimal):
    ...
    if START_MODE:
        ...
        # Gå in baserat på var priset är relativt L
        START_MODE = False
        if price > L:
            # Priset är ÖVER L → SHORT (förvänta reversion ner till L)
            enter_short(price)
            print(f"🎯 START: Priset över L ({L:.2f}) → SHORT (mean reversion)")
            return
        else:
            # Priset är UNDER L → LONG (förvänta reversion upp till L)
            enter_long(price)
            print(f"🎯 START: Priset under L ({L:.2f}) → LONG (mean reversion)")
            return
...
ax.set_title(f"{SYMBOL} – Markov REVERSION (Mean Reversion Paper)", fontsize=12, fontweight='bold')
ax.set_xlabel("Ticks")
ax.set_ylabel("Price")
